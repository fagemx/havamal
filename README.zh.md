...
執行狀態、控制平面、lane 並行——**這些不是 havamal 的工作**：那是狀態管理，該交給運行中的軟體。用 [edda](https://github.com/fagemx/edda)（事實、決策、協調自動流動），havamal 只管判斷層。舊的手養 markdown 方法保留在 [`docs/archive/`](docs/archive/) 供考古，勿當現行方法。
